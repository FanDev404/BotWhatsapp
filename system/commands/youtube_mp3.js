...
module.exports = {
    ...
    callback: async ({ sock, m }) => {
        ...
        const { status, data, message } = await youtubeDL(link, "mp3")
        if (!status) return m.reply(message) 
        if (parseInt(data.size) > 20000) return m.reply("Sizenya gede banget kak 🙂")
        sock.sendMessage(m.chat, { audio: { url: data.link }, mimetype: "audio/mp4", ptt: false }, { quoted: m })
    }
}
