    ...
    callback: async ({ mywa, m, cmdSuccess, cmdFailed, command, isImage, isQuotedImage, isQuotedSticker }) => {
        try {
            if (isImage || isQuotedImage) {
                ...
                const { status, base64, buffer, message } = await editBackground(media) 
                if (!status) return m.reply(util.format(message))
                await m.reply(buffer, m.chat, db.settings.replyType, [], m.sender, { type: "document", fileName: "MywaJS BOT.png", quoted: m })
                ...
                exec(`ffmpeg -i ${media} ./temp/${fileId}.jpg`, async (err) => {
                    const { status, base64, buffer, message } = await editBackground(fs.readFileSync(`./temp/${fileId}.jpg`).toString("base64")) 
                    if (!status) return m.reply(util.format(message))
                    await mywa.sendMessageV2(m.chat, { sticker: buffer })
                    cmdSuccess(command, "converter menu")
                }) 
            }
        } catch (error) {
