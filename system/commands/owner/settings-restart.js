...
export default {
    ...
    callback: async ({ m, command, cmdSuccess, cmdFailed }) => {
        try {
            if (!db.settings.local_restart.status) return m.reply("Future auto restart belum aktif kak") 
            ...
            m.reply(`Success set restart ${m.text}, Restaring bot...`)
            db.settings.local_restart.time = m.text
            setTimeout(() => {
                process.send("reset")
            }, 5000)
